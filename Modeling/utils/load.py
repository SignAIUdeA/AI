...
def load_encoder_methodology(df,val_split_label):
    """
    df: dataframe with the file names and the class names
    size: size of the cut of the data
    val_split_label: label of the validation data -> M1, M2, M3, M4, M5
    ...
    with Bar("Processing...", max = df.size) as bar:
        for file_name, class_name in df.iterrows():
            if val_split_label in file_name:
                X_val_list.append(np.load(f"../Clean_Data/{class_name.iloc[0]}/{file_name}")[:,:])
                X_val_list.append(np.load(f"../Data_Augmented_Clean/{class_name.iloc[0]}/{file_name}")[:,:])
                y_val_list.append(class_name.iloc[0])
                y_val_list.append(class_name.iloc[0])
            else:
                X_list.append(np.load(f"../Clean_Data/{class_name.iloc[0]}/{file_name}")[:,:])
                X_list.append(np.load(f"../Data_Augmented_Clean/{class_name.iloc[0]}/{file_name}")[:,:])
                y_list.append(class_name.iloc[0])
                y_list.append(class_name.iloc[0])
